...
public class VoteWebMvcTest {
	...
	public void insertTest() throws Exception {
		...
		mvc.perform(post("/api/v1/vote/making").contentType(MediaType.APPLICATION_JSON)
	...
	public void readTest() throws Exception {
		...
		VoteResponseDto responseDto = VoteResponseDto.builder().applyId(1L).id(1L).name("투표명").description("수정")
				.count(10).current(10).voteList(null).build();
		given(voteService.getVoteListByApplyId(Mockito.any(User.class), Mockito.anyLong())).willReturn(responseDto);

		mvc.perform(get("/api/v1/vote/making/1")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1L))
				.andExpect(jsonPath("$.applyId").value(1L)).andExpect(jsonPath("$.name").value("투표명"))
				.andExpect(jsonPath("$.description").value("수정")).andExpect(jsonPath("$.count").value(10))
				.andExpect(jsonPath("$.current").value(10)).andDo(print());
	}
	...
	public void voteTest() throws Exception {
		...
		votingList
				.add(VotingDto.builder().voteInfoId(1L).voteId(1L).candidateSequenceNumber(1L).candidateId(1L).build());
		...
		given(voteService.vote(Mockito.anyString(),Mockito.anyString(), Mockito.anyString(),Mockito.anyList()))
				.willReturn(ResponseEntity.created(URI.create("/vote/success")).build());

		System.out.println(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(votingList));

		mvc.perform(post("/api/v1/vote/abc/voting").session(session).contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsString(votingList))).andDo(print()).andExpect(status().isCreated());
	}
